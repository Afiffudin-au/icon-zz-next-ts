...
export default function Drawer() {
  ...
  const [alignmentStrokeType, setAlignmentStrokeType] = useState('')
  ...
  const handleChangeStrokeType = (
    event: React.MouseEvent<HTMLElement>,
    newAlignment: string
  ) => {
    setAlignmentStrokeType(newAlignment)
  }
  ...
  const handleFilter = (anchor: any, condition: any) => {
    ...
    query.strokeType = alignmentStrokeType
    if (userText === '') {
    ...
    if (query.strokeType === null || query.strokeType === '')
      delete query.strokeType
  ...
  const handleClearFilter = () => {
    ...
    setAlignmentStrokeType('')
    router.push(`/search-icons/${router.query.query}`, undefined, {
      shallow: true,
    })
  }
    ...
    <Box sx={matches ? { width: 300 } : { width: 340 }} role='presentation'>
      <Box p='10px' display='flex' flexDirection='column' gap='10px'>
        ...
        <ToggleButtonGroup
          fullWidth
          color='primary'
          value={alignmentStrokeType}
          exclusive
          onChange={handleChangeStrokeType}>
          <ToggleButton value='1'>filled</ToggleButton>
          <ToggleButton value='2'>linear</ToggleButton>
        </ToggleButtonGroup>
        <ToggleButtonGroup
          fullWidth
          color='primary'
